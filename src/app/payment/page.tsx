...
function PaymentContent() {
  ...
  useEffect(() => {
    (async () => {
      try {
        if (!platformSlug || !communityId) {
          toast.error("Missing platform or community information");
          setLoading(false);
          return;
        }
        ...
        console.error("Failed to load community info", e);
        toast.error("Failed to load community information");
      } finally {
        setLoading(false);
      }
    })();
  }, [platformSlug, communityId]);
    ...
    <div className="space-y-6">
      ...
      <div className="mx-auto w-full max-w-3xl space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground">Unlock access to {community.name}</p>
        </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="pt-6 border-t space-y-4">
            {(community.promoCode || community.promoCode2 || community.promoCode3) && (
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      const basePrice = community.monthlyPrice || 2999000;
                      let discount = 0;
                      let matchedPromo = null;
                      
                      // Check promo code 1
                      if (community.promoCode && inputCode === community.promoCode.toUpperCase()) {
                        matchedPromo = {
                          type: community.promoCodeType || "percentage",
                          value: community.promoCodeValue || 0
                        };
                      }
                      // Check promo code 2
                      else if (community.promoCode2 && inputCode === community.promoCode2.toUpperCase()) {
                        matchedPromo = {
                          type: community.promoCode2Type || "percentage",
                          value: community.promoCode2Value || 0
                        };
                      }
                      // Check promo code 3
                      else if (community.promoCode3 && inputCode === community.promoCode3.toUpperCase()) {
                        matchedPromo = {
                          type: community.promoCode3Type || "percentage",
                          value: community.promoCode3Value || 0
                        };
                      }
                      
                      if (matchedPromo) {
                        if (matchedPromo.type === "percentage") {
                          discount = Math.round((basePrice * matchedPromo.value) / 100);
                        } else {
            ...
            <div className="pt-4">
              ...
              >
                {processing ? (
                  ...
                  `Complete Payment • Rp ${(Math.max(0, (community.monthlyPrice || 2999000) - appliedDiscount)).toLocaleString('id-ID')}`
                )}
              </Button>
